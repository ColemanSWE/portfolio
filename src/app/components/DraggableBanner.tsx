...
export default function DraggableBanner() {
  const [position, setPosition] = useState({ x: 50, y: 900 });
  ...
  useEffect(() => {
    ...
    const safeY = Math.max(
      100,
      Math.min(500, window.innerHeight - bannerHeight - 50),
    );
  }, []);
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging) {
        ...
        const bannerHeight =
          ...
          Math.min(80, window.innerHeight * 0.12);

        const newX = Math.max(
          10,
          Math.min(
            e.clientX - dragOffset.x,
            window.innerWidth - bannerWidth - 10,
          ),
        );
        const newY = Math.max(
          10,
          Math.min(
            ...
            window.innerHeight - bannerHeight - 10,
          ),
        );
